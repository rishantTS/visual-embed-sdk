...
Allows adding rules for conditional formatting of data point on a chart or table.
|
...
|===
|Action string in SDK| Required SDK library|Action label in the UI
|`Action.Share`|`AppEmbed` | *Share* action in the *Data* > *Home* page+
Allows sharing a saved Answer with another user or group.
|`Action.Remove`|`AppEmbed` | *Delete* action in the *Data* > *Home*  and *Data* > *Connections* page.+
Allows deleting an Answer.
|`Action.ExportTML`| `AppEmbed` | *Export TML* action in the *Data* > *Home* page +
Allows exporting a saved Answer as a TML object.
|`Action.EditTML`| `AppEmbed` | *Edit TML*  action in the *Data* > *Home* page +
Opens the TML Editor that allows you to modify the properties of an Answer object.
|===
