...
In cookieless authentication, your app can use bearer tokens obtained from Thoughtpot instead of cookies to authenticate a user.  This method doesn’t generate or set a JSESSIONID cookie in API calls. Instead, the bearer token stored in the app memory is used to authenticate API calls to ThoughtSpot.
