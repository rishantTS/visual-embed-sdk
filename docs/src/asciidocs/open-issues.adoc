...
|SCAL-147773| The `Export Liveboard Report` API endpoint in the REST API v2.0 Playground returns the incorrect payload error.| *Symptom*: The `/api/rest/2.0/report/liveboard` endpoint in the Playground returns an error.

*Condition*: This issue occurs when a user tries to download a Liveboard in the PMG format via REST API in the v2.0 Playground. +
