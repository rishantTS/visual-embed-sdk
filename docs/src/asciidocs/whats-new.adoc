...
Starting with 9.2.0.cl, ThoughtSpot supports brushing and linking of visualizations on a Liveboard using cross filters. Cross filters allow you to present a coordinated view of a Liveboard by applying filters across all visualizations based on the current selection. Cross filters is an Early Access feature in 9.2.0.cl and is turned off by default. To enable this feature on your instance, contact ThoughtSpot Support.
...
ThoughtSpot now supports generating secret keys per Org. Tenant administrators can generate a secret key for trusted authentication in their Org context and allow Org users to obtain authentication tokens using this secret key.

Starting from 9.2.0.cl, Org users require Org-specific authentication tokens issued for their user accounts to authenticate and log in to ThoughtSpot. When generating an authentication token via REST API calls, make sure to request a separate token for each Org context.
