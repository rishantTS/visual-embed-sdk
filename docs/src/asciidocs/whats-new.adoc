...
Org users authenticating to ThoughtSpot via Trusted authentication method will require Org-specific tokens to switch between Orgs. When generating an authentication token on behalf of a user via `POST /tspublic/v1/session/auth/token` REST API call, the cluster administrator or Org administrator must make sure to request a token for each Org to which the user belongs.
