...
Starting with 9.2.0.cl, ThoughtSpot supports brushing and linking of visualizations on a Liveboard using cross filters. Cross filters allow you to present a coordinated view of a Liveboard by applying filters across all visualizations based on the current selection.

[NOTE]
====
The Cross filters feature is turned off by default in 9.2.0.cl. To enable this feature on your instance, contact ThoughtSpot Support.
====

==== Contextual menu behavior

For embedded instances with cross filters feature enabled, ThoughtSpot provides an option to set the contextual menu on visualizations as right-click or left-click pop-up menu.

If the contextual menu is set as left-click menu, and your app uses `EmbedEvent.VizPointClick` to listen to click events on an embedded Liveboard visualization, you will notice that the `VizPointClick` event is triggered whenever a user clicks a contextual menu action, such as *Drill down* or *Filter*. If you are using `EmbedEvent.VizPointClick` to trigger actions such as updating runtime filters on a Liveboard visualization, the click events from left-click contextual menu may impact your app’s current workflow.

To avoid this issue, you can use the following settings in the Visual Embed SDK v1.21.0 and later:

Define your preferred settings for the contextual menu::
You can use the `ContextMenuTrigger` property in the SDK to trigger the contextual menu on right-click or left-click. This will allow your app to fire the `VizPointClick` event as per your current workflow.

Turn off cross-filters::
You can turn off the cross-filters feature by setting `Action.CrossFilter` and `Action.RemoveCrossFilter` as hidden actions in the SDK. For more information about hiding a menu action, see xref:embed-actions.adoc[Show or hide menu items].

Use VizPointRightClick to monitor right-click events::
If the contextual menu is set as left-click menu, use the `EmbedEvent.VizPointRightClick` event to listen to the right-click events on a chart or table.
